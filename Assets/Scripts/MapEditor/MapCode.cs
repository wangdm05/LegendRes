...
public class MapReader
{
    ...
    public int PixelWidth
    {
        get
        {
            return Width * MapEditor.CellWidth;
        }
    }
    ...
    public int PixelHeight
    {
        get
        {
            return Height * MapEditor.CellHeight;
        }
    }
        ...
        try
        {
            ...
            for (int x = 0; x < Width; x++)
                for (int y = 0; y < Height; y++)
                {
                    ...
                    MapCells[x, y].BackIndex = 100;//改成盛大了
                    //  MapCells[x, y].MiddleIndex = 1;
                    MapCells[x, y].MiddleIndex = 110;//改成盛大了
                    MapCells[x, y].BackImage = (short)BitConverter.ToInt16(Bytes, offset);

                    MapCells[x, y].MiddleImage = (short)BitConverter.ToInt16(Bytes, offset += 2);//2
                    MapCells[x, y].FrontImage = (short)BitConverter.ToInt16(Bytes, offset += 2);//2

                    MapCells[x, y].DoorIndex = (byte)(Bytes[offset += 2] & 0x7F);//  [271 257]====7  2279542
                    // byte temp=Bytes[offset += 2];
                    //if (((temp & 0x80) == 0x80 )&& temp > 0) 
                    // {
                    //     MapCells[x, y].DoorIndex = (byte)(temp & 0x7F);//  [271 257]====7  2279542
                    // }
                    //else
                    //{
                    ...
                    MapCells[x, y].DoorOffset = Bytes[offset += 1];         //1
                    MapCells[x, y].FrontAnimationFrame = Bytes[offset += 1];//1 
                    MapCells[x, y].FrontAnimationTick = Bytes[offset += 1];          //1

                    //  MapCells[x, y].FrontIndex = (short)(Bytes[offset++] + 2);
                    MapCells[x, y].FrontIndex = (short)(Bytes[offset += 1] + 120);//1

                    MapCells[x, y].Light = Bytes[offset += 1];//1
                    offset += 1;                              //1
